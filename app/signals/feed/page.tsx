"use client";
// app/signals/feed/page.tsx — Live signal feed from smart money wallets
...
import { FeedLoadingState } from "@/components/loading-state";
...
export default function FeedPage() {
  ...
  if (isLoading || !data) return <FeedLoadingState />;
            ...
            signals.map((sig, i) => {
              ...
              return (
                <div key={i} style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.04)", display: "flex", alignItems: "flex-start", gap: "16px" }}>
                  <div style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "0.08em", padding: "3px 7px", borderRadius: "4px", flexShrink: 0, marginTop: "2px", background: `${typeColor}14`, color: typeColor, border: `1px solid ${typeColor}30` }}>
                    {sig.signal_type}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" as const }}>
                      <span style={{ fontSize: "13px", fontWeight: 600, color: "#f0f0f0" }}>{meta?.label ?? sig.recipe_id}</span>
                      ...
                      {sig.direction && <span style={{ fontSize: "10px", fontWeight: 700, color: dirColor, letterSpacing: "0.08em" }}>{sig.direction}</span>}
                    </div>
                  ...
                  {sig.ev_score != null ? (
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontSize: "15px", fontWeight: 700, color: sig.ev_score > 0.6 ? "#6aaa7a" : sig.ev_score > 0.3 ? "#f59e0b" : "#9ca3af" }}>{(sig.ev_score * 100).toFixed(0)}</div>
