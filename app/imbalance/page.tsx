"use client";
// app/imbalance/page.tsx
// Smart Money vs. Retail divergence scanner + rule-based Smart Trade Plan.
...
function ImbalanceInner() {
  ...
  if (isLoading || !data) return <div style={{ padding: "28px", ...S.muted }}>Computing imbalances...</div>;
  ...
  return (
    <>
      <PageHeader
        title="Imbalance"
        subtitle="Smart Money vs. Retail, live divergence scanner"
      />
      ...
      <div style={{ display: "grid", gridTemplateColumns: selected ? "1fr 1fr" : "1fr", gap: "16px" }}>
        ...
        <div style={S.card}>
          <div style={S.hdr}>
            <span style={S.label}>Imbalance Scanner</span>
            <span style={{ ...S.muted, marginLeft: "auto" }}>{data.ideas.length} imbalances detected</span>
          </div>
          {data.ideas.length === 0 ? (
            <div style={{ padding: "48px", textAlign: "center", ...S.muted }}>
              No significant imbalances at current threshold
            </div>
          ) : (
            data.ideas.map((idea) => {
          ...
          <div style={S.card}>
            ...
            {selectedIdea.trade_plan ? (
              <div style={{ padding: "20px" }}>
                ...
                {[
                  { label: "Entry Zone",    value: selectedIdea.trade_plan.entry_zone,   color: "#f0f0f0" },
                  { label: "Stop Loss",     value: selectedIdea.trade_plan.stop_loss,    color: "#b06868" },
                  { label: "Take Profit",   value: selectedIdea.trade_plan.take_profit,  color: "#6aaa7a" },
                  { label: "Risk / Reward", value: selectedIdea.trade_plan.risk_reward,  color: "#f0f0f0" },
