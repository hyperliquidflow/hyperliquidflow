"use client";
// app/wallets/leaderboard/page.tsx — Smart money leaderboard, top 50 with show-all
...
export default function LeaderboardPage() {
  ...
  if (isLoading || !data) return <LeaderboardLoadingState />;

  const sorted = [...data.top_wallets].sort((a, b) => {
  ...
  return (
    <>
      <PageHeader title="Leaderboard" subtitle={`Smart Money ranked by composite score · ${data.wallet_count} active`} />
        ...
        <div style={S.card}>
          ...
          {!showAll && sorted.length > 50 && (
            <button onClick={() => setShowAll(true)}
              style={{ width: "100%", padding: "14px", fontSize: "12px", color: "rgba(255,255,255,0.38)", background: "transparent", border: "none", borderTop: `1px solid ${color.divider}`, cursor: "pointer", transition: "color 0.15s" }}
