...
const S = {
  ...
  th:    { fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" as const, color: "rgba(227,227,227,0.38)", padding: "12px 16px", textAlign: "left" as const },
  ...
  return (
    <div style={S.page}>
      <div style={{ marginBottom: "20px" }}>
        <h1 style={{ fontSize: "18px", fontWeight: 700, color: "#e3e3e3" }}>Full Whale Report</h1>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
        {(["overall_score", "account_value", "unrealized_pnl", "win_rate"] as SortKey[]).map((k) => (
          <button key={k} onClick={() => setSort(k)} style={{
            ...
            background: sort === k ? "rgba(96,96,96,0.2)" : "transparent",
            border: `1px solid ${sort === k ? "rgba(96,96,96,0.5)" : "rgba(180,180,180,0.12)"}`,
            borderRadius: "5px", color: sort === k ? "#e3e3e3" : "rgba(227,227,227,0.38)",
            cursor: "pointer",
          }}>
          ...
          <tbody>
            {displayed.map((w, i) => (
              <tr key={w.wallet_id} style={{ transition: "background 0.1s" }}
                ...
                <td style={{ ...S.td, color: "rgba(227,227,227,0.28)", width: "40px" }}>{i + 1}</td>
                <td style={S.td}>
                ...
                <td style={S.td}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <div style={{ width: "48px", height: "3px", background: "rgba(180,180,180,0.1)", borderRadius: "2px" }}>
                      <div style={{ width: `${w.overall_score * 100}%`, height: "100%", background: w.overall_score >= 0.7 ? "#4ade80" : w.overall_score >= 0.5 ? "#606060" : "#f87171", borderRadius: "2px" }} />
                    </div>
                    <span style={{ fontSize: "11px", fontWeight: 600, color: "#e3e3e3" }}>{w.overall_score.toFixed(2)}</span>
                  </div>
                </td>
                <td style={{ ...S.td, fontVariantNumeric: "tabular-nums" }}>{formatUsd(w.account_value)}</td>
                <td style={{ ...S.td, color: w.unrealized_pnl >= 0 ? "#4ade80" : "#f87171", fontVariantNumeric: "tabular-nums" }}>
                  {formatUsd(w.unrealized_pnl)}
                </td>
                <td style={S.td}>{w.win_rate != null ? formatPct(w.win_rate) : "—"}</td>
                <td style={S.td}>{w.position_count}</td>
                <td style={{ ...S.td, color: (w.liq_buffer_pct ?? 1) < 0.15 ? "#f87171" : "rgba(227,227,227,0.6)" }}>
