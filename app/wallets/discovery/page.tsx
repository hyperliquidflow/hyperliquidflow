"use client";
// app/wallets/discovery/page.tsx — Wallet discovery stats + inline lookup
        ...
        <div style={{ marginLeft: "auto", display: "grid", gridTemplateColumns: "repeat(4,auto)", gap: "24px" }}>
          {[
            ...
            <div key={label} style={{ textAlign: "right" }}>
              ...
              <div style={{ fontSize: "14px", fontWeight: 700, color: c ?? "#f0f0f0", fontVariantNumeric: "tabular-nums" }}>{value}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "10px" }}>
        {[
          ...
          <div key={label} style={{ ...S.card, padding: "16px" }}>
            ...
            <div style={{ fontSize: "18px", fontWeight: 700, color: c ?? "#f0f0f0", marginTop: "6px", fontVariantNumeric: "tabular-nums" }}>{value}</div>
          </div>
        ))}
      </div>
      <div style={S.card}>
        <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
          {(["overview", "positions", "trades"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={`tab-hover${tab === t ? " tab-active" : ""}`}
              style={{ padding: "12px 20px", fontSize: "13px", fontWeight: 500, color: tab === t ? "#f0f0f0" : "rgba(255,255,255,0.44)", background: "none", border: "none", cursor: "pointer", borderBottom: tab === t ? "2px solid #606060" : "2px solid transparent", marginBottom: "-1px", textTransform: "capitalize" as const }}>
              {t}
            </button>
          ))}
        </div>
        ...
        {tab === "positions" && (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr>{["Coin","Side","Size","Value","uPnL","Entry Px","Lev","Liq Px"].map(h => <th key={h} style={{ padding: "10px 14px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" as const, color: "rgba(255,255,255,0.44)", textAlign: "left" as const }}>{h}</th>)}</tr></thead>
            <tbody>
        ...
        {tab === "trades" && (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr>{["Time","Coin","Side","Price","Size","Realized PnL"].map(h => <th key={h} style={{ padding: "10px 14px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" as const, color: "rgba(255,255,255,0.44)", textAlign: "left" as const }}>{h}</th>)}</tr></thead>
            <tbody>
              {profile.fills30d.slice(0, 50).map((f, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td style={{ ...S.td, color: pnl > 0 ? "#6aaa7a" : pnl < 0 ? "#b06868" : "rgba(255,255,255,0.44)", fontVariantNumeric: "tabular-nums" }}>{pnl !== 0 ? formatUsd(pnl) : "n/a"}</td>
                  </tr>
                );
              })}
            </tbody>
...
function DiscoveryInner() {
  ...
  // Handle ?address= param (supports links from leaderboard)
  useEffect(() => {
        ...
        <div style={{ ...S.card, padding: "16px 20px", marginBottom: "20px" }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            <input
              ...
              style={{ flex: 1, background: "#141414", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "6px", color: "#f0f0f0", fontFamily: "var(--font-mono)", fontSize: "13px", padding: "10px 14px", outline: "none" }}
            />
            <button onClick={() => doLookup(lookupAddr)} disabled={lookupLoading} className="glow-btn"
              style={{ padding: "0 20px", borderRadius: "7px", cursor: lookupLoading ? "not-allowed" : "pointer", background: "linear-gradient(135deg, #1e1e1e 0%, #2e2e2e 100%)", border: "1px solid rgba(255,255,255,0.1)", color: "#f0f0f0", fontSize: "13px", fontWeight: 700, boxShadow: "0 4px 20px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.08)", opacity: lookupLoading ? 0.5 : 1 }}>
              {lookupLoading ? "Loading..." : "Lookup"}
            </button>
          </div>
          {lookupError && <div style={{ color: "#b06868", fontSize: "12px", marginTop: "10px" }}>{lookupError}</div>}
        </div>
                ...
                <tbody>
                  {scannerData.top_win_rates.map((w, i) => (
                    <tr key={w.address}>
                      ...
                      <td style={{ ...S.td, color: w.win_rate >= 0.65 ? "#6aaa7a" : w.win_rate >= 0.55 ? "#f59e0b" : "#f0f0f0", fontWeight: 600 }}>{formatPct(w.win_rate)}</td>
...
export default function DiscoveryPage() { return <DiscoveryInner />; }
