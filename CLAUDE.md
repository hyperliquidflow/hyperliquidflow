...
- **The trade `side` is the aggressor's**, measured on 131 fills joined by `tid` with zero exceptions: an address's own fill side matches the tape exactly when its `crossed` flag is true. Position in `users` explains nothing. So signed **per-coin** flow is sound (it is aggressive buy against aggressive sell notional) and signed **per-address** flow is not derivable, because the trade never says which counterparty crossed. `flow_address_minute.side_b_notional` therefore means "notional this address was in while the aggressor was buying", not "notional this address bought". See [docs/research/2026-08-14-trade-side-convention.md](docs/research/2026-08-14-trade-side-convention.md).
